...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("glass-panel rounded-[1.5rem] border border-border/70 shadow-[0_30px_80px_-50px_rgba(15,23,42,0.45)]", className)}
      {...props}
    />
  )
);
...
const CardFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} className={cn("flex items-center px-6 pb-6", className)} {...props} />
);
